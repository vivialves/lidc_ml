...
NUM_AUG_PER_SAMPLE_TRAIN = 1
NUM_AUG_PER_SAMPLE_TEST_VAL = 1
...
num_aug_per_sample = NUM_AUG_PER_SAMPLE_TRAIN
...
class_counts = Counter(labels_train)
max_class_count = max(class_counts.values())
num_aug_per_sample = NUM_AUG_PER_SAMPLE_TEST_VAL
...
model.eval()
with torch.no_grad():
    for inputs, labels in test_loader:  # or test_loader
        inputs = inputs.to(device)
...
plt.grid(True)
filepath = os.path.join(PATH_RESULTS, f"ROC_Curve_Test.png")
plt.savefig(filepath)
plt.show()
...
plt.legend(loc="lower right")
plt.grid(True)
filepath = os.path.join(PATH_RESULTS, f"ROC_Curve_Val.png")
plt.savefig(filepath)
plt.show()
